# This Module has the Functions that allows a User to do certain tasks
...
def available_trains():
    """
    available_trains() -> Shows the List of Available Trains according to the User Requirement

    Parameters -> None   
    """

    mn = mysql.connector.connect(host="localhost", user=os.getenv('DB_USERNAME'),
                                 password=os.getenv('DB_PASSWORD'), database="railway")
    cur = mn.cursor()
    ...
    cur.execute(
        'SELECT Train_No, Source_Station_Name, Destination_Station_Name, Arrival_Time, Departure_Time from '
        'train_info where Source_Station_Code="{}" AND Destination_Station_Code="{}";'.format(
            start_opt, final_opt))
    result = cur.fetchall()
    os.system("cls")
    time.sleep(1)
...
def check_fare():
    """
    check_fare() -> Calculates the Fare based on the Distance
    ...
    cur.execute(
        'SELECT Train_No, Distance from train_info where Source_Station_Code="{}" AND '
        'Destination_Station_Code="{}";'.format(
            start_opt, final_opt))
...
def show_bookings():
    """
    show_bookings() -> Show the Bookings Made by a User
    ...
    if len(result) == 0:
        ...
        print(["Train_No", "Passenger_Name", "Mobile_No",
               "Passenger_Aadhar", "Time_Of_Booking", "Booking_ID", "Class"])
        for x in result:
...
def book_train():
    """
    book_train() -> Allow a User Book a Train
    ...
    while True:
        try:
            aadhar = int(input("Enter you Aadhar Number: "))
        except ValueError:
            print("Please Enter a Valid Aadhar Number!")
            continue
        else:
            if len(str(aadhar)) == 12 and aadhar != 000000000000:
                break
            elif len(str(aadhar)) > 12 or len(str(aadhar)) < 12:
        ...
        if ask in ["Y", "y"]:
            ...
            try:
                query = "INSERT INTO bookings values({}, '{}', '{}', '{}', '{}', {}, '{}')".format(
                    train_no, Name, Mobile, aadhar, date, id, Class)
                cur.execute(query)
            except DataError:
...
def cancel_booking():
    """
    cancel_booking() -> Allows a User to Cancel Booking
    ...
    while True:
        try:
            ...
            if len(str(unique_id)) == 0:
                ...
                print("ID Out of Range!")
            elif len(str(unique_id)) != 0 and 1 <= unique_id <= 10000:
                cur.execute(
                ...
                if len(result) == 0:
                    ...
                    break
                print(["Train_No", "Passenger_Name", "Mobile_No",
                       "Passenger_Aadhar", "Time_Of_Booking", "Booking_ID"])
                for x in result:
                    print(x)
                while True:
